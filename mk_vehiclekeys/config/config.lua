...
------------------------------------------------------DISPATCH----------------------------------------------------------------
Config.PoliceNotify = {
    Chance = 30, --Number 0-100. Lower the number less chance to alert. 0 = will never alert. 100 = will always alert.
...
Config.ProgressCirclePosition = 'bottom' --position of the progress circle. can be either 'middle' or 'bottom'
------------------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------LOCKPICKING CONFIG-------------------------------------------------------
Config.Lockpick = {
    LockpickIgnition = false, --TRUE = USING A LOCKPICK ITEM WILL LOCKPICK IGNITION
    IgnitionTime = {Min = 15, Max = 30}, --RANDOM AMOUNT OF TIME (SECONDS) TO ATTEMPT LOCKPICKING A VEHICLE DOOR WITH A REGULAR LOCKPICK. ADVANCED PICK WILL BE HALF OF THIS VALUE
    IgnitionFailChance = 10, --NUMBER 0-100. LOWER NUMBER = LESS CHANCE TO FAIL THE ATTEMPT. 0 = NEVER FAIL. 100 = ALWAYS FAIL.
    IgnitionBreakPick = {
        OnSuccess = {
            Break = false, --TRUE = ON SUCCESSFUL IGNITION LOCKPICK THE PICK HAS A CHANCE TO BREAK BASED ON NUMBER BELOW / FALSE = NEVER BREAK ON SUCCESS
    ...
    Game = { --SETUP GAMES TO USE FOR LOCKPICKING
        VehicleDoors = {
            UseGame = true, --TRUE = USE CUSTOM MINIGAME TO LOCKPICK VEHICLE DOORS / FALSE = USE DEFAULT PROGRESS BAR
            StartGame = function(AdvancedLockpick, cb) --CUSTOM CODE BELOW TO RUN YOUR MINI GAME (AdvancedLockpick will equal true/false depending on the item used) cb(true) for a game win and cb(false) for a game lose. IF YOU DONT SETUP CB CORRECTLY THE SCRIPT WILL HANG WAITING FOR A RESULT.
                local skills = exports['ForcngxAnnoying_misc']:GetSkills()
                local hasLockpicking1 = skills and skills.Lockpicking1 and skills.Lockpicking1.owned
                local hasLockpicking2 = skills and skills.Lockpicking2 and skills.Lockpicking2.owned
                local hasLockpicking3 = skills and skills.Lockpicking3 and skills.Lockpicking3.owned

                if AdvancedLockpick then 
                    Result = lib.skillCheck({'easy', 'easy', 'easy'})
                else
                    if hasLockpicking3 then
                        Result = lib.skillCheck({'medium', 'easy', 'easy'})
                    elseif hasLockpicking2 then
                        Result = lib.skillCheck({'easy', 'medium', 'medium'})
                    elseif hasLockpicking1 then
                        Result = lib.skillCheck({'medium', 'hard', 'easy'})
                    else
                        Result = lib.skillCheck({'hard', 'hard', 'hard'})
                    end
                end
                if Result then 
                    cb(true) 
        ...
        Ignition = {
            UseGame = true, --TRUE = USE CUSTOM MINI GAME TO LOCKPICK VEHICLE IGNITION / FALSE = USE OX_LIB SKILLCHECK MINIGAME
            StartGame = function(AdvancedLockpick, cb) --CUSTOM CODE BELOW TO RUN YOUR MINI GAME (AdvancedLockpick will equal true/false depending on the item used) cb(true) for a game win and cb(false) for a game lose. IF YOU DONT SETUP CB CORRECTLY THE SCRIPT WILL HANG WAITING FOR A RESULT.
                local skills = exports['ForcngxAnnoying_misc']:GetSkills()
                local hasLockpicking1 = skills and skills.Lockpicking1 and skills.Lockpicking1.owned
                local hasLockpicking2 = skills and skills.Lockpicking2 and skills.Lockpicking2.owned
                ...
                if AdvancedLockpick then 
                    ...
                    if hasLockpicking3 then
                        Result = lib.skillCheck({'easy', 'easy', 'medium'})
                    elseif hasLockpicking2 then
                        Result = lib.skillCheck({'easy', 'medium', 'medium'})
                    elseif hasLockpicking1 then
                        Result = lib.skillCheck({'medium', 'medium', 'hard'})
                    else
                        Result = lib.skillCheck({'medium', 'hard', 'hard'})
                    end
                end
                if Result then 
                    cb(true) 
...
------------------------------------------------------HOTWIRING CONFIG--------------------------------------------------------
Config.EnableHotwiring = false --TRUE = HOTWIRING ENABLED / FALSE = HOTWIRING DISABLED
...
Config.HotwireTextUI = { --EDIT THE TEXTUI THAT APPEARS TO SHOW YOU CAN HOTWIRE A VEHICLE
    Position = 'right-center', --left-center / right-center / top-center
    Icon = {
...
------------------------------------------------------NO SHUFFLE--------------------------------------------------------------
Config.NoShuffle = true --TRUE = RUN CHECKS WHEN IN PASSENGER SEAT TO NOT AUTO SHUFFLE TO DRIVERS SEAT / FALSE = DISABLED
------------------------------------------------------------------------------------------------------------------------------
...
Config.NpcVehicles = { --NONE OF THE CHECKS BELOW APPLY TO PLAYER OWNED OR STOLEN NPC VEHICLES.
    Driving = {
        ...
        LockedChance = 20, --NUMBER 0 TO 100. HIGHER NUMBER IS HIGHER CHANCE TO BE LOCKED. 100 = ALWAYS LOCKED. 0 = ALWAYS UNLOCKED.
    },
